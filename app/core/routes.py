...
# =============================================================================
# Benchmark Arena Endpoints
# =============================================================================

class BenchJoinRequest(BaseModel):
    api_key: str
    nickname: str
    expected_voters: int = 1

class BenchMessageRequest(BaseModel):
    message: str

class BenchVoteRequest(BaseModel):
    agent_alias: str

@router.get("/benchmark")
async def benchmark_dashboard():
    """Serve the Benchmark Arena HTML."""
    from fastapi.responses import HTMLResponse
    from app.core.benchmark_html import BENCHMARK_HTML
    return HTMLResponse(content=BENCHMARK_HTML)

@router.post("/api/benchmark/join")
async def benchmark_join(req: BenchJoinRequest):
    from app.core.benchmark import join_session
    try:
        token = join_session(req.api_key, req.nickname, req.expected_voters)
        return {"token": token}
    except ValueError as e:
        from fastapi import HTTPException
        raise HTTPException(status_code=401, detail=str(e))

@router.get("/api/benchmark/poll")
async def benchmark_poll(token: str = Header(None)):
    from fastapi import HTTPException
    from app.core.benchmark import get_poll_state
    
    if not token: raise HTTPException(401)
    
    state = get_poll_state(token)
    if not state:
        raise HTTPException(status_code=401, detail="Session expired")
    return state

@router.post("/api/benchmark/send")
async def benchmark_send(req: BenchMessageRequest, token: str = Header(None)):
    from fastapi import HTTPException
    from app.core.benchmark import get_poll_state, process_turn
    
    if not token or not get_poll_state(token): raise HTTPException(401)
    
    # Run in background or await? 
    # If we await, the user waits for generation. 
    # The UI shows "Thinking..." immediatey on button click, 
    # but actual state change happens inside process_turn.
    # We should await it so the response confirms it's done/voting started.
    await process_turn(req.message)
    return {"status": "ok"}

@router.post("/api/benchmark/vote")
async def benchmark_vote(req: BenchVoteRequest, token: str = Header(None)):
    from fastapi import HTTPException
    from app.core.benchmark import cast_vote
    
    if not token: raise HTTPException(401)
    cast_vote(token, req.agent_alias)
    return {"status": "ok"}

@router.post("/api/benchmark/reveal")
async def benchmark_reveal(token: str = Header(None)):
    from fastapi import HTTPException
    from app.core.benchmark import reveal_results
    
    if not token: raise HTTPException(401)
    reveal_results(token)
    return {"status": "ok"}

@router.post("/api/benchmark/next")
async def benchmark_next(token: str = Header(None)):
    from fastapi import HTTPException
    from app.core.benchmark import next_turn
